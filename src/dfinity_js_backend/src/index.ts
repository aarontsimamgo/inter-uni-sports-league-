...
export default Canister({
  ...
  assignCoachToTeam: update(
    ...
    (payload) => {
      ...
      // Check if the role is Coach using the variant structure
      if (!("Coach" in coach.role)) {
        return Err({
  ...
  scheduleMatch: update([MatchPayload], Result(Match, ErrorType), (payload) => {
    ...
    // Ensure that the teams dont play against themselves
    if (homeTeamOpt.Some.id === awayTeamOpt.Some.id) {
      return Err({ InvalidPayload: "Teams cannot play against themselves." });
    }

    // Ensure that the teams are playing the same sport
    // Fetch both teams' sport type
    const homeTeam = teamsStorage.get(payload.homeTeamId).Some;
    const awayTeam = teamsStorage.get(payload.awayTeamId).Some;

    const homeTeamSportType = homeTeam.sportType;
    const awayTeamSportType = awayTeam.sportType;

    // Debugging
    console.log(homeTeamSportType);
